...
from services.comparison_service import DecisionComparator
from services.regret_calculator import (
    calculate_regret, get_regret_level, get_regret_color, get_regret_explanation
)
from services.counterfactual_service import CounterfactualExplorer, generate_counterfactual_summary
from models.scenario import ScenarioResult, OutcomeMetrics, AnalysisResult, RiskFlag
from utils.visualizations import (
    format_percentage, get_confidence_color, get_emoji_for_scenario, get_severity_icon
)
from utils.charts import (
    plot_distribution, plot_scenario_comparison, plot_metric_distributions
)
...
def display_input_form():
    ...
    st.markdown('<div class="sub-header">Simulate Tomorrow. Decide Today.</div>', unsafe_allow_html=True)
    ...
    if analyze_button:
        if not decision_text or len(decision_text.strip()) < 10:
            st.error("Please provide a detailed description of your decision (at least 10 characters).")
        else:
...
def analyze_decision(decision_text: str, decision_type: str):
    ...
    try:
        ...
        status_text.info(f"🔄 Starting Monte Carlo simulation with {simulation_count:,} iterations...")
...
def display_risks(result: AnalysisResult):
    ...
    if result.risks:
        for risk in result.risks:
            ...
            st.warning(f"{severity_icon} **{risk.risk_type.replace('_', ' ').title()}** ({risk.severity.upper()})")
...
def display_sensitivity_analysis(result: AnalysisResult):
    ...
    if hasattr(st.session_state, 'structured_decision') and st.session_state.structured_decision:
        ...
        for idx, (var_name, var_dist) in enumerate(st.session_state.structured_decision.variables.items()):
            with cols[idx % 3]:
                weight = st.slider(
        ...
        st.info("💡 Sensitivity analysis re-calculation coming soon! This will show how changing variable weights affects outcomes.")
    else:
        st.info("Sensitivity analysis requires the original decision structure. Please run a new analysis.")


def display_chat_interface(result: AnalysisResult):
    """Display conversational chat interface."""
    st.subheader("💬 Discuss Your Decision")
    st.markdown("Ask questions, share your thoughts, or explore different aspects of your decision analysis.")
    ...
    with chat_container:
        ...
        if not st.session_state.chat_initialized or len(st.session_state.conversation_history) == 0:
            welcome_msg = f"Hi! I've completed the analysis of your decision. I can help you understand the results, discuss different scenarios, or explore any questions you have. What would you like to know?"
            st.session_state.conversation_history = [
...
def display_results(result: AnalysisResult):
    ...
    with col2:
        ...
        regret_display = f"{result.regret_score:.2f} ({regret_level})"
    ...
    with tab4:
        ...
        st.markdown("**What would change the outcome?** Explore what variable changes would shift scenarios.")
        ...
        if st.session_state.simulation_results and st.session_state.structured_decision:
            ...
            if counterfactuals:
                ...
                st.warning("Unable to compute counterfactuals. Please ensure sufficient simulation data.")
        else:
    ...
    with tab5:
        ...
        if st.session_state.structured_decision:
            ...
            fig = create_explainability_graph(
                st.session_state.structured_decision,
                causal_graph
            )
...
# Main app logic - Chat-based interface
def display_chat_main():
    """Main chat-based interface."""
    # Header
    st.markdown('<div class="main-header">🧠 Regretless AI</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-header">Simulate Tomorrow. Decide Today.</div>', unsafe_allow_html=True)
        ...
        if msg["role"] == "user":
            ...
            with st.chat_message("assistant"):
                ...
                if msg.get("type") == "analysis_result":
                    ...
                    st.markdown("---")
                    st.markdown("**You can now ask me questions about these results, request visualizations, or explore counterfactuals!**")
                
                elif msg.get("type") == "visualization":
                    # Display charts if requested
                    if msg.get("chart_type") == "distribution":
                        fig = plot_distribution(st.session_state.simulation_results, msg.get("metric", "overall_score"))
                        st.plotly_chart(fig, use_container_width=True)
        ...
        # If we have analysis results, generate conversational response
            ...
            with st.spinner("Thinking..."):
                try:
                    ...
                    if any(keyword in user_input.lower() for keyword in ["show me", "visualize", "chart", "graph", "plot"]):
                        if "distribution" in user_input.lower() or "histogram" in user_input.lower():
                            st.session_state.conversation_history.append({
                                "role": "assistant",
                                "content": assistant_response,
                                "type": "visualization",
...
# Main app logic - Always show chat interface
display_chat_main()
